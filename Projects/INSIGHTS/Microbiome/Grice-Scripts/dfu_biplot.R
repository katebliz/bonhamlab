...
funcDoBiplot <- function(
  ...
  sTitle = "Biplot of Taxa and Samples - Metadata Plotted with Centroids",
  ### The title for the figure.
  dfInput,
  ### Inpute data frame containing metadata followed by taxa/otu counts/proportions
  sLastMetadata,
  ### Last metadata that seperates data and metadata
  normalizeCounts = FALSE,
  ### Do you want the taxa/otu columns to be normalized to 1 for each sample?
  sOutputFileName
  ...
  ### Format table if necessary
  if(normalizeCounts) dfInput[,viData] <- t(apply(dfInput[,viData], 1, function(x) x/sum(x)))
  ...
  {
    vsRotateMetadata = sRotateByMetadata
    sMetadata = vsRotateMetadata[[1]]
    dWeight = vsRotateMetadata[[2]]
  ...
  mNMDSData = metaMDS(dfData,k=2,trymax = 999)
  ...
  if(fLegendMove)
  {
    pdf(sOutputFileName,useDingbats=FALSE, width=15, height=12)
  ...
  plot(mNMDSData$points, pch=vsShapes, 
       xlab=paste("NMDS1","Stress=",mNMDSData$stress), 
       ylab="NMDS2", 
       #col=vsColors) 
       col=c('blue','red','forestgreen','orange')[as.integer(dfInput$dmn)]) 
  title(sTitle,line=3)
  # Plot Bugs
...
dfu.sub <- filter(dfu, has_sample, SubjectID != 'Control')
taxa.sub <- apply(taxa$Species, 2, function(x) x/sum(x))
taxa.sub <- taxa.sub[rowSums(taxa.sub > 0) >= floor(ncol(taxa.sub) * 0.1),]
taxa.sub <- taxa.sub[,dfu.sub$SampleID]
#df <- cbind(dfu.sub, t(taxa$Species)) %>% 
df <- cbind(dfu.sub, t(taxa.sub)) %>% 
  filter(SubjectID != 'Control') %>%
  select(-SubjectID, -SampleID, -WellPosition, -SubjOrder)
# remove zero taxa after filtering to baseline
# df <- filter(df, Visit == 0)
# del <- which(colSums(df[,-c(1:83)]) == 0) + 83
# df <- df[,-del]

# Select metadata
# sMetadata <- c('meantisox','dmn_1','dmn_2','dmn_3','dmn_4',
#               'depth','surarea','UlcerDur','hgba1c','crp','ulcerloc_cat_Forefoot',
#               'ulcerloc_cat_Midfoot','ulcerloc_cat_Hindfoot','esr',
#               'v.abx_TRUE','EOSReas_Unhealed','EOSReas_Healed',
#               'EOSReas_Amputation','Offloading_TCC','Offloading_DH Boot',
#               'RawReads','Reads','Anaerobes','crp','wbc','glucose','v.b4.abx_TRUE',
#               'v.wd_TRUE','v.post.abx_TRUE')
sMetadata <- c('meantisox','dmn_1','dmn_2','dmn_3','dmn_4',
               'depth','surarea','UlcerDur','hgba1c','crp','esr',
               'v.abx_TRUE','EOSReas_Unhealed','EOSReas_Healed',
               'Anaerobes','crp','wbc','glucose','EOSReas_Amputation')


# Make plot
funcDoBiplot(sBugs = c('s__aureus','g__Streptococcus','s__pettenkoferi',
                       'g__Corynebacterium','g__Anaerococcus'),
             sMetadata = sMetadata,
             sColorBy = NULL,
             sColorRange = c("orange","cyan"),
             sTextColor = "black",
             ...
             sOutputFileName = './paper_plots/biplot.pdf'
)